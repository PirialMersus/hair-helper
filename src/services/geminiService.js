...
Если мастер говорит про финансы (например, "коворкинг 640", "аренда 1000", "купила шампунь 500"), классифицируй это как "finance" с типом "expense". Если сумма — это оплата от клиента, тип "income".

Не задавай лишних вопросов, если суть сообщения ясна. Если данных для какого-то поля нет, просто оставь его пустым или null в JSON, не нужно переспрашивать, если это не критично для записи.

Если мастер просит ПЕРЕНЕСТИ запись, генерируй ДВА действия: "cancel_appointment" для старого времени и "appointment" для нового.
  ...
  "actions": [
    {
      ...
      "data": {
        "name": "имя клиента (если применимо)",
        "date": "YYYY-MM-DD",
        "time": "HH:mm",
        "amount": 1000,
        "type": "income/expense",
        "category": "категория (услуга, аренда, материалы и т.д.)",
        "item": "название товара или суть расхода"
      }
    }
  ],
  "userMessage": "Короткое подтверждение на русском языке (например: 'Понял, записал расход на аренду 600р')"
}`;
